...
public class Tracers extends Module {
    ...
    public void onRender(Event3DRender event) {
        ...
        entities.stream()
                ...
                .filter(entity -> (entity.isInvisible() && getSettings().get(8).toToggle().state) || !entity.isInvisible())
                .filter(entity -> !(entity instanceof PlayerEntity && entity.getPos().distanceTo(mc.player.getPos()) > Objects.requireNonNull(ModuleManager.getModule(Tracers.class)).getSettings().get(1).toSlider().getValue()))
                .filter(entity -> !(entity instanceof EnderCrystalEntity && entity.getPos().distanceTo(mc.player.getPos()) > Objects.requireNonNull(ModuleManager.getModule(Tracers.class)).getSettings().get(3).toSlider().getValue()))
                .filter(entity -> !((EntityUtils.isAnimal(entity) || EntityUtils.isNeutral(entity) || EntityUtils.isHostile(entity)) && entity.getPos().distanceTo(mc.player.getPos()) > Objects.requireNonNull(ModuleManager.getModule(Tracers.class)).getSettings().get(7).toSlider().getValue()))
