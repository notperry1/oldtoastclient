	...
	public static void downloadSongs(boolean log) {
		try {
			FileUtils.copyURLToFile(
					  ...
					  ToastFileMang.getDir().resolve("notebot").resolve("songs.zip").toFile());
			ZipFile zip = new ZipFile(ToastFileMang.getDir().resolve("notebot").resolve("songs.zip").toFile());
			Enumeration<? extends ZipEntry> files = zip.entries();
			int count = 0;
			while (files.hasMoreElements()) {
				count++;
				ZipEntry file = files.nextElement();
				File outFile = ToastFileMang.getDir().resolve("notebot").resolve(file.getName()).toFile();
			...
			zip.close();
			Files.deleteIfExists(ToastFileMang.getDir().resolve("notebot").resolve("songs.zip"));

			if(log) ToastLogger.infoMessage("Downloaded " + count + " Songs");
		} catch (Exception e) { if(log) ToastLogger.warningMessage("Error Downloading Songs... " + e); e.printStackTrace(); }
	}
